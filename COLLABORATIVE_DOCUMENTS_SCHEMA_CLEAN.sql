-- =====================================================
-- COLLABORATIVE DOCUMENTS SCHEMA - CLEAN INSTALLATION
...
DROP POLICY IF EXISTS "Owners and editors can update documents" ON documents;
...
DROP POLICY IF EXISTS "Editors can update content" ON document_content;
DROP POLICY IF EXISTS "Members can view workspace collaborators" ON workspace_collaborators;
DROP POLICY IF EXISTS "Users can view document collaborators" ON document_collaborators;
...
DROP POLICY IF EXISTS "Users can add comments to accessible documents" ON document_comments;

-- Drop existing triggers
DROP TRIGGER IF EXISTS update_documents_updated_at ON documents;
DROP TRIGGER IF EXISTS update_document_content_updated_at ON document_content;
DROP TRIGGER IF EXISTS trigger_cleanup_presence ON document_presence;

-- Drop existing functions (but not update_updated_at_column - it's shared)
-- DROP FUNCTION IF EXISTS update_updated_at_column(); -- SKIP: Used by other tables
DROP FUNCTION IF EXISTS cleanup_old_presence();
DROP FUNCTION IF EXISTS create_document(UUID, TEXT, TEXT, UUID, JSONB);
DROP FUNCTION IF EXISTS add_document_collaborator(UUID, UUID, TEXT, UUID);
DROP FUNCTION IF EXISTS create_revision_snapshot(UUID, UUID, TEXT);
...
DROP INDEX IF EXISTS idx_document_content_doc_id;
DROP INDEX IF EXISTS idx_collaborators_workspace;
DROP INDEX IF EXISTS idx_collaborators_user;
DROP INDEX IF EXISTS idx_doc_collaborators_document;
DROP INDEX IF EXISTS idx_doc_collaborators_user;
DROP INDEX IF EXISTS idx_revisions_document;
DROP INDEX IF EXISTS idx_revisions_created;
DROP INDEX IF EXISTS idx_presence_document;
...
CREATE INDEX IF NOT EXISTS idx_doc_collaborators_document ON document_collaborators(document_id);
...
-- =====================================================
-- Note: Using DO block to safely add tables to realtime publication
DO $$
BEGIN
  -- Add documents to realtime (if not already added)
  BEGIN
  ...
  -- Add document_content to realtime (if not already added)
  BEGIN
  ...
  -- Add document_presence to realtime (if not already added)
  BEGIN
  ...
  -- Add document_comments to realtime (if not already added)
  BEGIN
...
DO $$
BEGIN
  RAISE NOTICE '✅ Collaborative documents schema applied successfully!';
  RAISE NOTICE 'Tables created: documents, document_content, document_collaborators, etc.';
  RAISE NOTICE 'Now refresh your workspace page to create documents!';
END $$;
