-- =========================================
-- WORKSPACE SCHEMA - Clean Installation
...
DROP POLICY IF EXISTS "Workspace owners can manage members" ON workspace_users;
DROP POLICY IF EXISTS "Users can view papers in their workspaces" ON workspace_papers;
DROP POLICY IF EXISTS "Workspace members can add papers" ON workspace_papers;

-- Drop existing triggers if they exist
DROP TRIGGER IF EXISTS add_workspace_owner_trigger ON workspaces;
DROP TRIGGER IF EXISTS update_workspace_updated_at_trigger ON workspaces;
...
DO $$
BEGIN
  RAISE NOTICE '✅ Workspace schema applied successfully!';
  RAISE NOTICE 'Tables created: workspaces, workspace_users, workspace_papers';
  RAISE NOTICE 'Next: Apply COLLABORATIVE_DOCUMENTS_SCHEMA.sql';
END $$;
