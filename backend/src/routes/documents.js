...
// =========================================
// COLLABORATIVE WORKSPACE DOCUMENTS
// =========================================

// Get all collaborative documents in a workspace
router.get('/workspaces/:workspaceId/collaborative-documents', requireAuth, async (req, res) => {
  try {
    const { workspaceId } = req.params;
    const userId = req.user.id;
    ...
    const { data: documents, error } = await supabase
      ...
      .select(`
        ...
        created_by,
        workspace_id,
        author:users!created_by(name, email)
      `)
      .eq('workspace_id', workspaceId)
      .order('updated_at', { ascending: false });
...
router.post('/workspaces/:workspaceId/collaborative-documents', requireAuth, async (req, res) => {
  try {
    const { workspaceId } = req.params;
...
router.put('/workspaces/:workspaceId/collaborative-documents/:documentId', requireAuth, async (req, res) => {
  try {
...
router.post('/workspaces/:workspaceId/collaborative-documents/:documentId/collaborators', requireAuth, async (req, res) => {
  try {
    const { workspaceId, documentId } = req.params;
    ...
    // Add collaborator to workspace if not already a member
    ...
    if (!existingMembership) {
      await supabase
        ...
        .insert({
          workspace_id: workspaceId,
          user_id: user.id,
          role: permission === 'viewer' ? 'viewer' : 'member'
        });
    }
